...
\begin{itemize}
    ...
    Calculate the neuron errors for the output layers.
    Calculate the neuron errors for the hidden layers.
    Calculate the weight gradients and the bias gradients.
    Update the network parameters using the average gradients.
    Save the updated network parameters to their respective files.
\end{itemize}
...
\begin{enumerate}
    ...
    \centering
        ...
        \hline
        \textbf{Input} & \textbf{Expected outcome} & \textbf{Observed outcome} \\\hline
        Clicking the `Profile' button on the main menu & The profile page is shown & \\\hline
        Clicking the `Profile' button on the main menu & Each letter from A-Z is viewable with all statistics relating to it shown & \\\hline
...
As for the interviewee's experience with teaching young children how to write, they stated that they believe the solution would be effective for those learning pencil control. While the end user for this project was not young children, this statement may stil imply that the solution is also effective for those able to write, but are learning a new script. I myself found that a similar application from Analysis is convinenient to use and effective at its purpose. There are many similarities between learning to write and learning to write in a different script, hence this feedback suggests that the produced solution is effective in solving the proposed problem.
...
% // how would you then implement this?
...
% // add some mising labels to file column

\begin{table}[H]
    \centering
        \begin{tabularx}{\textwidth}{|L|L|L|}
        \hline
        \textbf{File} & \textbf{Description} & \textbf{Pages} \\\hline

        server/games/Game.cs/IPlayable, Game & Object-Oriented Programming & page \\\hline
        server/databases/Database.cs/LoadUserData() & Loading account data using paramterised SQL & page \\\hline
        server/neuralNetwork/Backpropagation.cs/Backpropagate() & Backpropagation algorithm & page \\\hline
        server/neuralNetwork/Data.cs/LoadWeights(), SaveWeights() & Reading and writing to text files & \\\hline
        server/games/Versus.cs/CalculateRank() & Implementation of the Elo formulae & page \\\hline
        server/games/Elimination/EvaluationPhase() & List operations for determining which users are eliminated & page \\\hline
        server/games/Game.cs/ & Evaluating a submission using the neural network & page \\\hline
        server/games/Game.cs/UpdateStatistics() & Updating averages in the database & page \\\hline
        server/neuralNetwork/Data.cs/softmax() & Softmax function & page \\\hline
        server/neuralNetwork/Data.cs/PreprocessImage() & Image preprocessing & page \\\hline
        server/neuralNetwork/Network.cs/EvaluateNetwork() & Forward pass of the neural network & page \\\hline
        server/Accounts.cs & Account management & \\\hline
        server/Program.cs/ & Server configuration & page \\\hline

        client/menus/Main.cs/CalculateStatsOverview() & Calculating a summary of a user's statistics & page \\\hline
        client/menus/Main.Designer.cs/ConfigGamePanels() & Configuring the panels to display the available game types & page \\\hline
        client/hub-connection.cs/AddHandles() & Client logic for receiving server invokations & page \\\hline
...
% ~ embedded links in pdf OKAY
% ~ do i specifically have enough diagrams PROBABLY
% ~ have i sufficiently explained the neural network ??
% ~ do i need a test for every objective PROBABLY
% ~ do i need a whole system test YES
% ~ what if uml diagrams are a little inaccurate NO
% ~ is my testing video too fast NO
