...
The client also contains three main units, each of which represents a major `page' which the user can visit in the application. The `Main' page is the screen which the user is first presented with upon logging into their account. From here, the user can queue for a game type of their choosing, or access their profile page to view their learning progress. The `Profile' page details a user's statistics for each letter of the Latin alphabet, including their average accuracy, average time, and the number of times which they have drawn each letter. The `Games' component manages the invocations received by the server when the progession of a game is advanced, usually involving updates to the screen. \\

\subsection{Choice of Language and Libraries}

Since object-oriented programming is a necessary paradigm to use, C\# will be the most suitable language for this project. This also allows for the use of Windows Forms to create the UI for the client, and SignalR to provide a client-server connection. I have significant experience in both of these software, hence why they are suitable for the technical solution. \\

Regarding the libraries required for the server application, the SignalR library is mandatory for creating a server which clients can connect to. Furthermore, the SQLite library will be used to store and retrieve the account data and statistics for each user, and allow the C\# codebase to interact with the database file. For the neural network, I have chosen to use the Mathnet.Numerics library to perform operations on matrices, since this library provides `Matrix' and `Vector' classes. These will be used to improve the speed of the forward pass of the network. \\

As for the libraries required by the client application, the user interface will be created using the Windows Forms library. I have also chosen to use the Guna.UI2.WinForms library, which provides an enhanced set of controls to use with Windows Forms, such as progress bars and gradient fill colours. Additionally, the client application also requires the SignalR.Client library to allow it to establish a connection with the server.
...
Backpropagation is the process by which the parameters for the network are optimised to accurately evaluate a user's submission. The \textbf{Backpropagation} class is constructed with a batch of training data, where the size of the batch is assigned to a variable. The weights and biases for the network are then loaded using the methods in the \textbf{Data} class. The list below outlines the general process for performing the backpropagation algorithm.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item \textbf{07:00} [4.a] A user can send a friend invite to another user.
        \item \textbf{07:06} Sending a friend request twice does not cause an error.
        \item \textbf{07:19} [4.a.i] A friend invite is forwarded to the client if they are currently online.
        \item \textbf{07:49} [4.a.i] A friend invite is forwarded to the client when they are next online if they are currently offline.
        \item \textbf{07:38} [4.a] A user can remove a friend, which is immediately updated for the other user.
        \item \textbf{07:32} Accepting a friend invite updates both users' friends list.
    \end{enumerate}
    ...
    \begin{enumerate}
        \item \textbf{07:55} [3.a] A user's profile page can be viewed which displays their statistics.
        \item \textbf{07:59} [3.a.i] The statistics shown on a user's profile page are separated by letter and include the average accuracy and average time for each letter.
        \item \textbf{08:07} [3.d] A user's profile can be searched for using their exact username.
        \item \textbf{08:16} [3.d] Searching for a user that does not exist returns a suitable error message.
    \end{enumerate}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item \textbf{08:25} [5.a] A user can connect to the server from localhost.
        \item \textbf{08:29} [5.a] A user can connect to the server from another device on the same subnet.\@
        \item \textbf{08:44} [5.a] A user can connect to the server from a different network over the internet.
    \end{enumerate}
...
Shown below is a table of contents which details any code files with complex or interesting logic. Each file is accompanied by a brief description of the functionality it provides, as well as the page number where the code can be found. It should be noted that the page number links to the start of the file, so the relevant method may be up to a few pages later.
...
\begin{table}[H]
    \centering
        ...
        client/menus/\newline Main.Designer.cs/ConfigGamePanels() & Configuring the panels to display the available game types; user interface logic &\pageref{client/menus/main.designer.cs} \\\hline
        client/\newline hub-connection.cs/AddHandles() & Client-server connection: client logic for receiving server invocationsm;  &\pageref{client/hub-connection.cs} \\\hline
...
\noindent\href{https://www.3blue1brown.com/topics/neural-networks}{Theory of neural networks ~ Grant Sanderson (Oct 2017)}
\noindent\href{http://neuralnetworksanddeeplearning.com/chap2.html}{How the backpropagation algorithm works ~ Michael Nielsen (Dec 2019)}
...
\noindent\href{https://grantwinney.com/using-async-await-and-task-to-keep-the-winforms-ui-more-responsive/}{Using Async, Await, and Task to keep the WinForms UI responsive ~ Grant Winney (Jun 2021)} \\
