...
It was stated in the Analysis section that I would be using an external library to perform the matrix operations required for the forward pass of the neural network. I have since decided against this, and have opted to create my own Matrix and Vector classes for this project. While these are likely less efficient than using a library, when creating the technical solution I did not notice any significant time delay while a submission was being evaluated.
...
As stated in the introduction to the Design section, the server application implements its own \textbf{Matrix} and \textbf{Vector} classes for the forward pass of the neural network. By using operator overloading, methods can be created which override the functionality of arithmetic operations. This is especially useful for these classes since their operations are performed in unintuitive ways, and it improves the legibility of the code. I have implemented the operations which are specifically required by the neural network such as multiplication of a vector and a matrix. Additionally, some similar operations to the required ones were also added for completeness. The values in the \textbf{Matrix} and \textbf{Vector} classes are stored by a private array, and access to these values is controlled by an index overriding subroutine. This allows the indexing property to be directly applied to the object reference for these classes, which would mirror an array data type better than using public `getters' and `setters' \\

Both of these classes implement public methods to get the dimensions of their stored data. The \textbf{Matrix} class contains two of these methods: one for each dimension, whereas the \textbf{Vector} class only contains one since it is one-dimensional. Additionally, both classes have a public method to extract the entire private array, which is used in obtaining the neuron values after a layer has been calculated in the forward pass of the network.

\paragraph{Matrices\\}

The \textbf{Matrix} class contains two constructors. The first constructor takes in two integers as arguments and creates an empty matrix with the specified dimensions. This is used when performing an operation on two matrix/vector classes, since the result of the calculation is first initialised to an empty matrix/vector. The second constructor is used when building a matrix object from an array of values, and therefore passes in an array as the constructor argument. \\

The first set of operator overloads present in the \textbf{Matrix} class are the addition and subtraction of two matrices. First a check is made to ensure that the matrices have dimensions such that the addition can be performed: the matrices must have identical dimensions. A new \textbf{Matrix} class is then instantiated with the same dimensions as the input matrices. The program iterates through each value of the resulting matrix using two embedded iteration structures, and sets the value to the either the sum or the difference of the corresponding values in the input matrices. \\

The remaining operator overloads are used to multiply matrices and vectors together. Since only the operation of multiplying a \textbf{Vector} by a \textbf{Matrix} is required, I will only document this method. However I have also implemented methods for multiplying the converse of this, and multiplying two matrices together for completeness. Similar to addition, the first check of the method is to ensure that multiplication can be performed on the inputs. This is done by checking if the length of the vector is equal to the number of rows in the matrix. A new \textbf{Vector} class is then instantiated, passing in the number of columns in the matrix as the parameter since this value will be equal to the length of the resulting vector from this operation. The program then iterates through each value in this new vector object, and initialises a new `sum' variable. Another iteration structure is started, which iterates through each row element of the current column in the matrix and multiplies it by equivalent value from the vector object. Each result of these multiplications is added to the `sum' variable, which is set to the appropriate value of the resulting vector object. The pseudocode for this process is outlined below:
...
The \textbf{Vector} class contains two constructors, similar to the \textbf{Matrix} class. The first constructor takes in one integer as an argument and creates an empty vector with the specified dimension. As with the \textbf{Matrix} class, this is used for the resulting vector when performing an operation on two matrices/vectors. The second constructor is used when building a vector object from an array of values, and therefore passes in an array as the constructor argument. \\

This class only contains one set of operators, since the operations involving both a matrix and a vector are found within the \textbf{Matrix} class. These operations are for the addition and subtraction of two vectors. Only the addition operation is used by the neural network, however subtraction works in almost an identical way. First, a check is made to see if the addition operation is valid for input vectors. This is done by comparing the lengths of the two vectors---if they are not equal then the calculation cannot be performed. Otherwise, a new \textbf{Vector} class is instantiated, passing in the length of one of the input vectors as its parameter. The program then iterates through each value of the resulting vector object, and sets each value to the sum of the corresponding values from each of the input vectors.
