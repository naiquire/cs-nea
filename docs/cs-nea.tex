...
The general process by which a game runs is by repeating two key phases until the condition for the game to be ended is met. When the game is started, the first `Submission' phase is started which is where a letter is broadcasted to the clients and the user's submissions are received by the server. Once all of the expected submissions have been received, the `Evaluation' phase is started where the server uses the neural network to evaluate each of the users' submissions. The results are then sent to the client application, and the server waits for all clients to acknowledge 
