...
\begin{enumerate}
    ...
    \item In this question they consider how a leaderboard could be ineffective for motivation. They state how a daily notification or reminder could encourage users to practise daily, so this is something that could be present in the solution.
    \begin{enumerate}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            \item An invitation will be forwarded to a user if they are online, otherwise it will be stored in a database.
        \end{enumerate}
...
In order to save and load the network parameters, I opted to store the values as plaintext. I formatted the values as separated by commas and line breaks, where each layer has its own file. For the weights, each line represents a single neuron where each comma separated value is an arc to a neuron in the next layer. The biases are stored in a single line, with commas separating the value for each neuron.
...
The `UpdateFriendData()' method is invoked by the server when a friend of the user updates their account data, or the user has added a friend. The method passes a `friendData' structure as a parameter, and iterates through each friend of the user. If one of these friends matches the userID in the friendData parameter, then the current friend is updated with the new structure. Otherwise, the friend must be a new friend, so the structure is added to the list of friends. The friends panel is then refreshed if the user is not in-game. \\
...
\begin{enumerate}
    ...
    These tests involve the login process to an existing account: `account1' which was set up prior to these tests. For each test, the input is entered into the `Username' and `Password' text boxes, and the `Login' button is clicked. The output is typically given by a red message beneath the `Password' text box.
...
Regarding the user experience while using the application, the client presents an appealing interface which is intuitive to use. Each section of the client application is split up, and navigating between these sections works flawlessly. Any exceptions which may occur on the server application are handled by the client by presenting relevant messages to the user, such that a runtime exception is completely avoided---both on the client and server application. A user's account data is stored securely in the database, along with their learning progress of the Latin characters. In my opinion, the technical solution I have produced acknowledges the proposed problem and provides an effective solution.
...
\begin{table}[H]
    \centering
        ...
        server/games/\newline Game.cs/EvaluateSubmission() & Evaluating a submission using the neural network &\pageref{server/games/game.cs} \\\hline
...
\end{minted}
\paragraph{neuralNetwork/structures/Matrix.cs}\label{server/neuralnetwork/structures/matrix.cs}
\begin{minted}{csharp}
...
\end{minted}
\paragraph{neuralNetwork/structures/Vector.cs}\label{server/neuralnetwork/structures/vector.cs}
\begin{minted}{csharp}
namespace server_app.neuralNetwork.structures
{
